...
export function activate(context: ExtensionContext) {
  ...
  const attributeValueProvider = languages.registerCompletionItemProvider("xml", {
    provideCompletionItems(document: TextDocument, position: Position) {
      ...
      const astPosition = astPositionAtOffset(ast, document.offsetAt(position));
      ...
      const attributeValue = astPosition.astNode.value;
      if (!attributeValue) {
        return undefined;
      }

      const completions: CompletionItem[] = [];
      const minifiedAttributeValue = attributeValue.replace(/\s+/g, '');
       ...
       */
      if (/\{.*\}/.test(minifiedAttributeValue)) {
        completions.push(...[
  ...
  context.subscriptions.push(attributeValueProvider);
}
